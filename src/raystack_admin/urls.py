...
async def user_edit_post(request: Request, user_id: int):
    ...
    user = await UserModel.objects.filter(id=user_id).first()
    if user:
        user.name = form.get("name") or ""
        # Handle empty values for numeric fields
        age_value = form.get("age")
        user.age = int(age_value) if age_value and age_value.strip() and age_value.strip() != "None" else None
        user.email = form.get("email") or ""
        user.organization = form.get("organization") or ""
        group_id_value = form.get("group_id")
        group_id = int(group_id_value) if group_id_value and group_id_value.strip() and group_id_value.strip() != "None" else None
        user.group = group_id
        await user.save()
    return render_template(request=request, template_name="admin/user_edit.html", context={
        "user": user,
...
async def user_create_post(request: Request):
    ...
    # Handle empty values for numeric fields
    age_value = form.get("age")
    age = int(age_value) if age_value and age_value.strip() and age_value.strip() != "None" else None
    
    group_id_value = form.get("group_id")
    group = int(group_id_value) if group_id_value and group_id_value.strip() and group_id_value.strip() != "None" else None
    ...
    user = UserModel(
        ...
        group=group,
        organization=form.get("organization") or ""
    )
    await user.save()
    return render_template(request=request, template_name="admin/user_create.html", context={
        "groups": await GroupModel.objects.all().execute(),
